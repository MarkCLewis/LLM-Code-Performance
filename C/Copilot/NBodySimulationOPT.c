...
KDTreePool* create_pool(int capacity) {
    KDTreePool *pool = (KDTreePool *)malloc(sizeof(KDTreePool));
    pool->nodes = (KDNode *)malloc(capacity * sizeof(KDNode));
    pool->index = 0;
    pool->capacity = capacity;
    return pool;
}

KDNode* allocate_node(KDTreePool *pool) {
    if (pool->index >= pool->capacity) {
        fprintf(stderr, "Pool capacity exceeded\n");
        exit(EXIT_FAILURE);
    }
...
void initialize_bodies(Body *bodies, int n) {
    ...
    for (int i = 1; i < n; i++) {
        ...
        bodies[i].vz = 0.0;
        bodies[i].mass = 1e24; // Small body mass
    }
}
...
KDNode* build_kdtree(Body *bodies, int n, int depth, KDTreePool *pool) {
    if (n <= 0) return NULL;
    ...
    qsort(bodies, n, sizeof(Body), (axis == 0 ? compare_x : (axis == 1 ? compare_y : compare_z)));

    KDNode *node = allocate_node(pool);
    node->body = &bodies[median];
    node->left = build_kdtree(bodies, median, depth + 1, pool);
    node->right = build_kdtree(bodies + median + 1, n - median - 1, depth + 1, pool);
