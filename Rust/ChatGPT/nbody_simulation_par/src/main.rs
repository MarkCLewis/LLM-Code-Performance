...
fn compute_forces(bodies: &mut Vec<Body>) {
    ...
    bodies.par_iter_mut().for_each(|bi| {
        bi.acc = Vec3::zero();
        for bj in &cloned_bodies {
            if !std::ptr::eq(bi, bj) {
                let dx = bj.pos.sub(&bi.pos);
                let dist2 = dx.norm_squared() + EPS2;
                let dist = dist2.sqrt();
                let force = G * bj.mass / (dist2 * dist);
                bi.acc = bi.acc.add(&dx.scale(force));
            }
        }
    });
}
