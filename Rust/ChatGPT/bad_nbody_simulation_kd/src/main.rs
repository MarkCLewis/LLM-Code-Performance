...
fn initialize_system(n: usize) -> Vec<Body> {
    ...
    for i in 1..=n {
        ...
        let r = radius * (1.0 + 0.1 * rng.r#gen::<f64>());
        let x = r * angle.cos();
        let y = r * angle.sin();
        let z = 0.0;
...
fn build_kdtree(bodies: &[Body], depth: usize) -> Option<Box<KDNode>> {
    if bodies.is_empty() {
        return None;
    }
    ...
    sorted.sort_by(|a, b| a.pos[axis].partial_cmp(&b.pos[axis]).unwrap());

    let mid = sorted.len() / 2;
    let node_bodies = sorted.clone();
    let min = Vec3 {
        x: node_bodies.iter().map(|b| b.pos.x).fold(f64::INFINITY, f64::min),
        y: node_bodies.iter().map(|b| b.pos.y).fold(f64::INFINITY, f64::min),
        z: node_bodies.iter().map(|b| b.pos.z).fold(f64::INFINITY, f64::min),
    };
    let max = Vec3 {
        x: node_bodies.iter().map(|b| b.pos.x).fold(f64::NEG_INFINITY, f64::max),
        y: node_bodies.iter().map(|b| b.pos.y).fold(f64::NEG_INFINITY, f64::max),
        z: node_bodies.iter().map(|b| b.pos.z).fold(f64::NEG_INFINITY, f64::max),
    };
    let total_mass: f64 = node_bodies.iter().map(|b| b.mass).sum();
    let cm = node_bodies
        .iter()
        .map(|b| b.pos.scale(b.mass))
        .fold(Vec3::zero(), |a, b| a.add(&b))
        .scale(1.0 / total_mass);

    let left = build_kdtree(&sorted[..mid], depth + 1);
    let right = build_kdtree(&sorted[mid..], depth + 1);
...
fn compute_force(b: &Body, node: &KDNode) -> Vec3 {
    if node.mass == 0.0 || node.bodies.iter().any(|bb| std::ptr::eq(bb, b)) {
        return Vec3::zero();
    }
    ...
    if node.bodies.len() <= 1 || (size * size / dist2 < THETA * THETA) {
        let dist = dist2.sqrt();
        let force = G * node.mass / (dist2 * dist);
        dx.scale(force)
    } else {
        let left = node.left.as_ref().map(|l| compute_force(b, l)).unwrap_or(Vec3::zero());
        let right = node.right.as_ref().map(|r| compute_force(b, r)).unwrap_or(Vec3::zero());
        left.add(&right)
    }
}

fn compute_forces(bodies: &mut Vec<Body>) {
    let tree = build_kdtree(bodies, 0).unwrap();
    bodies.par_iter_mut().for_each(|b| {
        b.acc = compute_force(b, &tree);
    });
}
...
// Helper for indexing Vec3 by axis
use std::ops::Index;
