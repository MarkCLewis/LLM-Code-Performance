...
impl NodePool {
    ...
    fn allocate(&mut self, center: [f64; 3], size: f64) -> &mut Node {
        if self.index >= self.pool.len() {
            self.pool.push(Node::new(center, size));
        } else {
            self.pool[self.index] = Node::new(center, size);
        }
        self.index += 1;
        &mut self.pool[self.index - 1]
    }
...
fn simulate_step(bodies: &mut [Body], pool: &mut NodePool) {
    // Reset forces
    ...
    // Build kD-tree
    pool.reset();
    let root = pool.allocate([0.0, 0.0, 0.0], 2.0 * 1e11);
...
fn main() {
    ...
    // Create a node pool
    let mut pool = NodePool::new(NUM_BODIES);
    ...
    for _ in 0..1000 {
        simulate_step(&mut bodies, &mut pool);
    }
