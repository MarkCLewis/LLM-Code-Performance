...
# Compute the force using the Barnes-Hut approximation
def compute_force(i, positions, masses, kdtree, theta, mass_central):
    # Get the position and mass of body i
    xi, yi, zi = positions[i]
    mi = masses[i]
    
    # Search for nearby bodies using the k-d tree (with a reasonable distance threshold)
    ...
    # Apply the Barnes-Hut approximation: if the bodies are far apart, treat them as a single cluster
    # Compute the center of mass and total mass of the cluster of bodies
    ...
    if total_mass > 0:
        ...
        # If the bodies are far enough, approximate their interaction using the center of mass
...
def run_simulation(positions, velocities, masses, mass_central, num_steps, time_step, theta):
    ...
    with ProcessPoolExecutor() as executor:
        for step in range(num_steps):
            ...
            # Recompute the k-d tree and forces at the new positions
            kdtree = KDTree(positions)
