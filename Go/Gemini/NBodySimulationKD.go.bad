...
// kickStepKDTree performs the "kick" step using the kD-tree for force calculation.
func kickStepKDTree(s *System, dt float64) {
	numBodies := len(s.Bodies)
...
func firstOrderKickStepKDTree(s *System, dt float64) {
	kickStepKDTree(s, dt)
	driftStep(s, dt)
	kickStepKDTree(s, dt)
}
...
func main() {
	...
	for step := 0; step < numSteps; step++ {
		firstOrderKickStepKDTree(system, timeStep)
		if (step+1)%100 == 0 {
