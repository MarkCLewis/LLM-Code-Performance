...
type KDTreePool struct {
	nodes    []*KDNode
	index    int
	capacity int
}

func createPool(capacity int) *KDTreePool {
	pool := &KDTreePool{
		nodes:    make([]*KDNode, capacity),
		index:    0,
		capacity: capacity,
	}
...
func allocateNode(pool *KDTreePool) *KDNode {
	if pool.index >= pool.capacity {
		fmt.Println("Pool capacity exceeded")
		return nil
	}
	node := pool.nodes[pool.index]
	pool.index++
	return node
}
...
func buildKDTree(bodies []Body, depth int, pool *KDTreePool) *KDNode {
	if len(bodies) == 0 {
		return nil
	}
	...
	median := len(bodies) / 2
	node := allocateNode(pool)
	node.body = &bodies[median]
	node.left = buildKDTree(bodies[:median], depth+1, pool)
	node.right = buildKDTree(bodies[median+1:], depth+1, pool)
...
func main() {
	...
	pool := createPool(NumBodies * 2) // Allocate pool with sufficient capacity

	for step := 0; step < 10; step++ {
		pool.index = 0 // Reset pool index for reuse
		root := buildKDTree(bodies, 0, pool)
