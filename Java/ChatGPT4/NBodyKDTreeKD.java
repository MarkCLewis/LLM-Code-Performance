...
public class NBodyKDTreeKD {
    ...
    static KDNode buildKDTree(List<Body> bodyList, int depth) {
        ...
        for (int i = 0; i < 3; i++) {
            node.min[i] = Double.POSITIVE_INFINITY;
            node.max[i] = Double.NEGATIVE_INFINITY;
        }
    ...
    static void computeForces(KDNode tree) {
        Arrays.stream(bodies).parallel().forEach(b -> {
            b.ax = b.ay = b.az = 0;
    ...
    static void computeForceFromNode(Body b, KDNode node) {
        ...
        if (node.bodyList.size() <= 1 || (size / dist < THETA)) {
    ...
    //     double kinetic = Arrays.stream(bodies).parallel().mapToDouble(b -> {
